...
public class Print extends com.global.BasicInputs {
  ...
  public static String format_price(float price) {
    ...
    String num, rev_str, formatted_rev = "", str_price; // Integer string // Integer string reversed // Integer string with formatted commas // Float String
    ...
    if (num_length <= 3) {
      return String.format(
        "%s%s",
        num,
        str_price.substring(num_length, num_length + 3)
      );
    }
    ...
    for (int i = 0; i < num_length; i += 3) {
      try {
        String sample = rev_str.substring(
          i,
          ((i + 3) >= num_length) ? num_length : i + 3
        );
